...
public class UserService implements UserDetailsService {
        ...
        if (looksLikeEmail) {
            return userRepository.findActiveUserByEmail(identifier)
                    .orElseThrow(() -> new UsernameNotFoundException(
                            "User not found or disabled with email: " + identifier));
        }
        return userRepository.findActiveUserByPhone(identifier)
                .orElseThrow(
                        () -> new UsernameNotFoundException("User not found or disabled with phone: " + identifier));
    }
    ...
    public UserResponseDto registerUser(UserRegistrationDto registrationDto) {
        ...
        // Create new user (referral is now handled at KVision purchase, not at
        // registration)
        User user = new User();
    ...
    public void deleteUser(Long id) {
        ...
        // CASCADE DELETE using native SQL queries
        ...
        // 2. Delete payment transactions for user's orders
        entityManager.createNativeQuery(
                "DELETE FROM payment_transactions WHERE order_id IN (SELECT id FROM orders WHERE user_id = :userId)")
                .setParameter("userId", id).executeUpdate();

        // 3. Delete coupon transactions (both as recipient and as source)
        entityManager
                .createNativeQuery(
                        "DELETE FROM coupon_transactions WHERE user_id = :userId OR source_user_id = :userId")
                .setParameter("userId", id).executeUpdate();

        // 4. Delete coin transactions (both as recipient and as source)
        entityManager
                .createNativeQuery("DELETE FROM coin_transactions WHERE user_id = :userId OR source_user_id = :userId")
                .setParameter("userId", id).executeUpdate();

        // 5. Delete referral relations (both as user and as parent)
        entityManager.createNativeQuery("DELETE FROM referral_relations WHERE user_id = :userId OR parent_id = :userId")
                .setParameter("userId", id).executeUpdate();
        ...
        // 8. Delete cart items
        entityManager.createNativeQuery("DELETE FROM cart_items WHERE user_id = :userId")
                .setParameter("userId", id).executeUpdate();

        // 9. Delete wishlist items
        entityManager.createNativeQuery("DELETE FROM wishlist_items WHERE user_id = :userId")
                .setParameter("userId", id).executeUpdate();
